...
//不等于
hbs.registerHelper("noequal", function(v1, v2, options) {
    if (v1 != v2) {
        //满足添加继续执行
        return options.fn(this);
    } else {
...
//是双数
hbs.registerHelper("double", function(index, options) {
    if ((index+1)%2 == 0) {
        //满足添加继续执行
        return options.fn(this);
    } else {
...
//对象不为空
hbs.registerHelper("notEmptyArray", function(obj, options) {
    if (obj.length > 0) {
        //满足添加继续执行
        return options.fn(this);
    } else {
