...
const sign = {
    ...
    getData: function(callBack) {
        var _this = this;
        var data = {};
        var url = '/app/user/GetSalt?ios_ver=2.36&iostype=0';
        Vue.http.post(STATIC_RUL + url, data).then(function(d) {
                let succValBody = JSON.parse(d.data);
                if (succValBody.msg_code == 9004) {
                    localStorage.set("varifyCode", succValBody.data);
                    localStorage.set("varifyTime", succValBody.server_time);
                }
                if (callBack) {
                    callBack();
                }
    ...
    setSign: function(data) {
        ...
        if (localStorage.get("varifyTime") && localStorage.get("varifyCode")) {
            var day = _this.timeConversion(localStorage.get("varifyTime"));
            var encryptstr = _this.encrypt(localStorage.get("varifyCode"), day);
