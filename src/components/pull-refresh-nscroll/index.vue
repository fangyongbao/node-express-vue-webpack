<template>
    <div class="m-pull-refresh-n">
        <div class="scroller" :class="{hasTransition: isDroping}" :style="{ 'transform': 'translate3d(0, ' + translateY + 'px, 0)' }">
            <div class="f-flex f-flextc f-flexvc pullDown" v-if="showPullDown">
                <span class="icon" :class="downClass" :style="{ 'transform': 'rotate3d(0, 0, 1, '+ deg +'deg)' }"></span>
            </div>
            <div>
                <slot></slot>
            </div>
            <div class='f-flex f-flextc f-flexvc pullUp' v-if="showPullUp">
    ...
    methods: {
        ...
        handleTouchMove(event) {
            ...
            if (_this.direction === 'down' && currentScrollTop == 0) {
                ...
                _this.translateY = (distance - _this.dis) / 2;
                ...
                deg = deg > 360 ? 360 : deg;
                _this.deg = deg;
            } else if (_this.direction === 'up' && (currentScrollTop + _this.wrapperTarget.clientHeight >= _this.wrapperTarget.scrollHeight)) {
                event.preventDefault();
                event.stopPropagation();
            }
            // 使用下拉刷新
